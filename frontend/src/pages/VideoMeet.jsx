...
export default function VideoMeetComponent() {
    ...
    // ✅ moved from module scope into the component, so it's fresh per mount
    // and doesn't leak across different meetings in the same tab.
    ...
    // ✅ fixed: was running on every render (no dependency array).
    useEffect(() => {
        console.log("HELLO")
        getPermissions();
    }, [])

    // ✅ new: cleanup when the component unmounts (e.g. user leaves this meeting).
    // Prevents stale peer connections, listeners, and chat state from leaking
    // into the next meeting joined in the same tab.
    useEffect(() => {
        return () => {
